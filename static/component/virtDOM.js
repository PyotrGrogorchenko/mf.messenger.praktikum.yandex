...
exports.Node = exports.VirtDom = void 0;
...
class VirtDom {
    ...
    init(parsedTemplate, state, props) {
        ...
        for (let i = 0; i < parsedTemplate.length; i++) {
            ...
            if (item.type === parser_1.PARSER_TYPES.CODE) {
                const iObj = { i };
    ...
    _compileItem(item, state, props) {
        switch (item.type) {
            case parser_1.PARSER_TYPES.END:
                this._closeTag();
                break;
            case parser_1.PARSER_TYPES.TEXT:
                this._addNode(item.content, item.type, state, props);
                break;
            case parser_1.PARSER_TYPES.BEGIN:
                this._addNode(item.content, item.type, state, props);
                break;
            default:
    ...
    _addNode(header, type, state, props) {
        if (type === parser_1.PARSER_TYPES.BEGIN) {
        ...
        else if (type === parser_1.PARSER_TYPES.TEXT) {
