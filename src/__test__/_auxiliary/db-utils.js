...
async function initDb({
  ...
  listItems = _.flatten(
    users.map(u => Array.from({ length: Math.floor(Math.random() * 4) }, () => generate.buildListItem({ ownerId: u.id, bookId: random(books).id })))
  )
} = {}) {
  await Promise.all([
...
export {
  resetDb, initDb, insertTestUser, generate
}
